...
# Data path/name
datapath = '../lowdosePETdata_30s_MS5.hdf5'
...
samp_im = np.c_[cond_samp[0,MSos,...,1],cond_samp[0,MSos,...,0],simfulldose_im,fulldose_im]
...
plt.plot(np.arange(numIter),-medfilt(dis_loss,5),
         np.arange(numIter),medfilt(1000*gen_loss[:,0],5),
...
multi_slice_viewer0(np.c_[x_test[:,MSos,...,1],x_test[:,MSos,...,0],test_output[...,0],y_test[...,0]],'Test Images',SSIMs)

test_output[test_output<0] = 0
y_mask = y_test[...,0]>.05
test_error = np.divide(np.abs(test_output-y_test),y_test,where=y_test!=0)
test_error = test_error[...,0]*y_mask
error1 = np.c_[x_test[:,MSos,...,0],test_error]
error2 = np.c_[y_test[...,0],test_output[...,0]]
multi_slice_viewer0(np.concatenate((error1,error2),axis=1),'Absolute Percent Error')
