...
def BlockModel3D(input_shape,filt_num=16,numBlocks=3):
    ...
    #calculate appropriate cropping
    mod = np.mod(input_shape[0:3],2**numBlocks)
    padamt = mod+2
    # calculate size reduction
    startsize = np.max(input_shape[0:3]-padamt)
    minsize = (startsize-np.sum(2**np.arange(1,numBlocks+1)))/2**numBlocks
    if minsize<4:
        raise ValueError('Too small of input for this many blocks. Use fewer blocks or larger input')
    
    crop = Cropping3D(cropping=((0,padamt[0]), (0,padamt[1]), (0,padamt[2])), data_format=None)(lay_input)
    ...
    lay_pad = ZeroPadding3D(padding=((0,padamt[0]), (0,padamt[1]),(0,padamt[2])), data_format=None)(lay_act)
        
    # regressor
    lay_out = Conv3D(1,(1,1,1), activation='linear',name='output_layer')(lay_pad)
