...
model_savepath = 'LowDosePET_WGAN_v1.hdf5'
# Data path/name
...
def GeneratorModel(samp_input):
    weight_init = RandomNormal(mean=0., stddev=5*weight_std)
...
def DiscriminatorModel(input_shape,test_shape,filtnum=16):
    ...
    weight_init = RandomNormal(mean=0., stddev=weight_std)
    weight_con = max_norm(weight_std/2,axis=[0,1,2])
    weight_conD = max_norm(weight_std/2,axis=0)
    ...
    lay_step = Conv2D(filtnum,(4,4),padding='valid',strides=(2,2),name='StepdownLayer',
                      ...
                      kernel_constraint=max_norm(weight_std,axis=[0,1,2]))(lay_input)
    # contracting block 1
    rr = 1
    lay_conv1 = Conv2D(filtnum*rr, (1, 1),padding='same',name='Conv1_{}'.format(rr),
                       kernel_initializer=weight_init,
                       kernel_constraint=weight_con)(lay_step)
    ...
    lay_conv52 = Conv2D(filtnum*rr, (3, 3),padding='same',name='Conv52_{}'.format(rr),
                        kernel_initializer=weight_init,
                       kernel_constraint=weight_con)(lay_conv51)
    lay_merge = concatenate([lay_conv1,lay_conv3,lay_conv52],name='merge_{}'.format(rr))
    lay_conv_all = Conv2D(filtnum*rr,(1,1),padding='valid',name='ConvAll_{}'.format(rr),
                          kernel_initializer=weight_init,
                       kernel_constraint=weight_con)(lay_merge)
    lay_bn = BatchNormalization()(lay_conv_all)
    lay_act = ELU(name='elu{}_1'.format(rr))(lay_bn)
    lay_stride = Conv2D(filtnum*rr,(3,3),padding='valid',strides=(2,2),name='ConvStride_{}'.format(rr),
                        kernel_initializer=weight_init,
                       kernel_constraint=weight_con)(lay_act)
    lay_bn = BatchNormalization()(lay_stride)
    lay_act1 = ELU(name='elu{}_2'.format(rr))(lay_bn)
    ...
    lay_step2 = Conv2D(filtnum,(4,4),padding='valid',strides=(2,2),name='StepdownLayer2',
                       kernel_initializer=weight_init,
                       kernel_constraint=weight_con)(lay_test)
    # contracting block 1
    rr = 1
    lay_conv1 = Conv2D(filtnum*rr, (1, 1),padding='same',name='Conv1_{}t'.format(rr),
                       kernel_initializer=weight_init,
                       kernel_constraint=weight_con)(lay_step2)
    ...
    for rr in range(2,6):
        ...
        lay_conv_all = Conv2D(filtnum*rr,(1,1),padding='valid',name='ConvAll_{}'.format(rr),
                              ...
                              kernel_constraint=weight_con)(lay_merge)
        lay_bn = BatchNormalization()(lay_conv_all)
        lay_act = ELU(name='elu_{}'.format(rr))(lay_bn)
        lay_stride = Conv2D(filtnum*rr,(3,3),padding='valid',strides=(2,2),name='ConvStride_{}'.format(rr),
                            kernel_initializer=weight_init,
                            kernel_constraint=weight_con)(lay_act)
        lay_bn = BatchNormalization()(lay_stride)
        lay_act = ELU(name='elu{}_2'.format(rr))(lay_bn)
    
    lay_flat = Flatten()(lay_act)
    lay_dense = Dense(1,activation='linear',name='realfake',
                      kernel_initializer=weight_init,
                      kernel_constraint=weight_conD)(lay_flat)
...
adoptD = optimizers.adam(lr=1e-5)
adoptG = optimizers.adam(lr=1e-5)

rmsoptD = optimizers.RMSprop(lr=1e-5)
rmsoptG = optimizers.RMSprop(lr=1e-5)

DisModel = DiscriminatorModel(x_train.shape[1:],y_train.shape[1:],20)
DisModel.compile(optimizer=adoptD,loss=wasserstein_loss)
...
GMmodel.compile(optimizer=adoptG, loss=wasserstein_loss)
...
for pp in t:
    ...
    batch_inds = np.random.choice(x_train.shape[0], size=b_s, replace=False)
    ...
    y_batchF = -softMax*np.ones([b_s, 1])
