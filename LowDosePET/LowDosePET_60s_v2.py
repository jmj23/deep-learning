...
numEp = 60  # Set maximum number of Epochs
b_s = 8 # Set batch size
...
def ResidualBlockModel(input_shape):
    ...
    # contracting block 1
    rr = 1
    lay_conv1 = Conv2D(fn*(2**(rr-1)), (1, 1),padding='same',name='Conv1_{}'.format(rr))(crop)
    lay_conv3 = Conv2D(fn*(2**(rr-1)), (3, 3),padding='same',name='Conv3_{}'.format(rr))(crop)
    lay_conv51 = Conv2D(fn*(2**(rr-1)), (3, 3),padding='same',name='Conv51_{}'.format(rr))(crop)
    lay_conv52 = Conv2D(fn*(2**(rr-1)), (3, 3),padding='same',name='Conv52_{}'.format(rr))(lay_conv51)
    lay_merge = concatenate([lay_conv1,lay_conv3,lay_conv52],name='merge_{}'.format(rr))
    lay_conv_all = Conv2D(fn*(2**(rr-1)),(1,1),padding='valid',name='ConvAll_{}'.format(rr))(lay_merge)
    bn = BatchNormalization()(lay_conv_all)
    lay_act = ELU(name='elu{}_1'.format(rr))(bn)
    lay_stride = Conv2D(fn*(2**(rr-1)),(4,4),padding='valid',strides=(2,2),name='ConvStride_{}'.format(rr))(lay_act)
    lay_act = ELU(name='elu{}_2'.format(rr))(lay_stride)
    act_list = [lay_act]
    
    # contracting blocks 2-4
    for rr in range(2,5):
        lay_conv1 = Conv2D(fn*(2**(rr-1)), (1, 1),padding='same',name='Conv1_{}'.format(rr))(lay_act)
        lay_conv3 = Conv2D(fn*(2**(rr-1)), (3, 3),padding='same',name='Conv3_{}'.format(rr))(lay_act)
        lay_conv51 = Conv2D(fn*(2**(rr-1)), (3, 3),padding='same',name='Conv51_{}'.format(rr))(lay_act)
        lay_conv52 = Conv2D(fn*(2**(rr-1)), (3, 3),padding='same',name='Conv52_{}'.format(rr))(lay_conv51)
        lay_merge = concatenate([lay_conv1,lay_conv3,lay_conv52],name='merge_{}'.format(rr))
        lay_conv_all = Conv2D(fn*(2**(rr-1)),(1,1),padding='valid',name='ConvAll_{}'.format(rr))(lay_merge)
        bn = BatchNormalization()(lay_conv_all)
    ...
    # expanding block 3
    dd=4
    lay_deconv1 = Conv2D(fn*(2**(dd-1)),(1,1),padding='same',name='DeConv1_{}'.format(dd))(lay_act)
    lay_deconv3 = Conv2D(fn*(2**(dd-1)),(3,3),padding='same',name='DeConv3_{}'.format(dd))(lay_act)
    lay_deconv51 = Conv2D(fn*(2**(dd-1)), (3,3),padding='same',name='DeConv51_{}'.format(dd))(lay_act)
    lay_deconv52 = Conv2D(fn*(2**(dd-1)), (3,3),padding='same',name='DeConv52_{}'.format(dd))(lay_deconv51)
    lay_merge = concatenate([lay_deconv1,lay_deconv3,lay_deconv52],name='merge_d{}'.format(dd))
    lay_deconv_all = Conv2D(fn*(2**(dd-1)),(1,1),padding='valid',name='DeConvAll_{}'.format(dd))(lay_merge)
    bn = BatchNormalization()(lay_deconv_all)
    lay_act = ELU(name='elu_d{}'.format(dd))(bn)
    
    lay_up = UpSampling2D()(lay_act)
    
    lay_cleanup = Conv2DTranspose(fn*(2**(dd-1)), (3, 3),name='cleanup{}_1'.format(dd))(lay_up)
    lay_act = ELU(name='elu_cleanup{}_1'.format(dd))(lay_cleanup)
    lay_cleanup = Conv2D(fn*(2**(dd-1)), (3,3), padding='same', name='cleanup{}_2'.format(dd))(lay_act)
    ...
    for dd in expnums:
        ...
        lay_deconv1 = Conv2D(fn*(2**(dd-1)),(1,1),padding='same',name='DeConv1_{}'.format(dd))(lay_skip)
        lay_deconv3 = Conv2D(fn*(2**(dd-1)),(3,3),padding='same',name='DeConv3_{}'.format(dd))(lay_skip)
        lay_deconv51 = Conv2D(fn*(2**(dd-1)), (3, 3),padding='same',name='DeConv51_{}'.format(dd))(lay_skip)
        lay_deconv52 = Conv2D(fn*(2**(dd-1)), (3, 3),padding='same',name='DeConv52_{}'.format(dd))(lay_deconv51)
        lay_merge = concatenate([lay_deconv1,lay_deconv3,lay_deconv52],name='merge_d{}'.format(dd))
        lay_deconv_all = Conv2D(fn*(2**(dd-1)),(1,1),padding='valid',name='DeConvAll_{}'.format(dd))(lay_merge)
        bn = BatchNormalization()(lay_deconv_all)
...
multi_slice_viewer0(np.c_[x_test[...,0],x_test[...,1],test_output[...,0],y_test[...,0]],'Test Results',SSIMs)

#Export to NIFTI
#import nibabel as nib
#testsubj1 = np.rot90(np.flip(np.flip(np.rollaxis(np.rollaxis(test_output[:89,...,0],2,0),2,0),0),1))
#output_img = nib.Nifti1Image(20000*testsubj1, np.eye(4))
#output_img.to_filename('subj014_simFullDosePET_60s.nii')
