...
from keras.layers import Cropping3D, Conv3D, Conv3DTranspose, ZeroPadding3D
...
#%% 3D Version
def BlockModel3D(input_shape,filt_num=16,numBlocks=3):
    # Input shape should in the form
    # [rows,cols,slices,channels]
    lay_input = Input(shape=(input_shape),name='input_layer')
    
    #calculate appropriate cropping
    mod = 2**numBlocks-np.mod(input_shape[0:3],2**numBlocks)
    padamt = mod-2
    # calculate size reduction
    startsize = np.min(input_shape[0:3]+padamt)
    minsize = (startsize-np.sum(2**np.arange(1,numBlocks+1)))/2**numBlocks
    if minsize<4:
        raise ValueError('Too small of input for this many blocks. Use fewer blocks or larger input')
    
    pad = ZeroPadding3D(padding=((0,padamt[0]), (0,padamt[1]), (0,padamt[2])), data_format=None)(lay_input)

    # contracting block 1
    rr = 1
    lay_conv1 = Conv3D(filt_num*rr, (1, 1, 1),padding='same',name='Conv1_{}'.format(rr))(pad)
    lay_conv3 = Conv3D(filt_num*rr, (3, 3, 3),padding='same',name='Conv3_{}'.format(rr))(pad)
    lay_conv51 = Conv3D(filt_num*rr, (3, 3, 3),padding='same',name='Conv51_{}'.format(rr))(pad)
    ...
    bn = BatchNormalization()(lay_conv_all)
    lay_act = ELU(name='elu{}_1'.format(rr))(bn)
    lay_stride = Conv3D(filt_num*rr,(4,4,4),padding='valid',strides=(2,2,2),name='ConvStride_{}'.format(rr))(lay_act)
    lay_act = ELU(name='elu{}_2'.format(rr))(lay_stride)
    act_list = [lay_act]
    ...
    dd=numBlocks
    lay_deconv1 = Conv3D(filt_num*dd,(1,1,1),padding='same',name='DeConv1_{}'.format(dd))(lay_act)
    lay_deconv3 = Conv3D(filt_num*dd,(3,3,3),padding='same',name='DeConv3_{}'.format(dd))(lay_act)
    lay_deconv51 = Conv3D(filt_num*dd, (3,3,3),padding='same',name='DeConv51_{}'.format(dd))(lay_act)
    lay_deconv52 = Conv3D(filt_num*dd, (3,3,3),padding='same',name='DeConv52_{}'.format(dd))(lay_deconv51)
    lay_merge = concatenate([lay_deconv1,lay_deconv3,lay_deconv52],name='merge_d{}'.format(dd))
    lay_deconv_all = Conv3D(filt_num*dd,(1,1,1),padding='valid',name='DeConvAll_{}'.format(dd))(lay_merge)
    bn = BatchNormalization()(lay_deconv_all)
    lay_act = ELU(name='elu_d{}'.format(dd))(bn)
    
    lay_up = UpSampling3D()(lay_act)    
    lay_cleanup = Conv3DTranspose(filt_num*dd, (3,3,3),name='cleanup{}_1'.format(dd))(lay_up)
    lay_act = ELU(name='elu_cleanup{}_1'.format(dd))(lay_cleanup)
    lay_cleanup = Conv3D(filt_num*dd, (3,3,3), padding='same', name='cleanup{}_2'.format(dd))(lay_act)
    bn = BatchNormalization()(lay_cleanup)
    lay_act = ELU(name='elu_cleanup{}_2'.format(dd))(bn)
    ...
    for dd in expnums:
        lay_skip = concatenate([act_list[dd-1],lay_act],name='skip_connect_{}'.format(dd))
        lay_deconv1 = Conv3D(filt_num*dd,(1,1,1),padding='same',name='DeConv1_{}'.format(dd))(lay_skip)
        lay_deconv3 = Conv3D(filt_num*dd,(3,3,3),padding='same',name='DeConv3_{}'.format(dd))(lay_skip)
        lay_deconv51 = Conv3D(filt_num*dd, (3,3,3),padding='same',name='DeConv51_{}'.format(dd))(lay_skip)
        lay_deconv52 = Conv3D(filt_num*dd, (3,3,3),padding='same',name='DeConv52_{}'.format(dd))(lay_deconv51)
        lay_merge = concatenate([lay_deconv1,lay_deconv3,lay_deconv52],name='merge_d{}'.format(dd))
        lay_deconv_all = Conv3D(filt_num*dd,(1,1,1),padding='valid',name='DeConvAll_{}'.format(dd))(lay_merge)
        bn = BatchNormalization()(lay_deconv_all)
    ...
    lay_crop = Cropping3D(cropping=((0,padamt[0]), (0,padamt[1]),(0,padamt[2])), data_format=None)(lay_act)
        
    # regressor
    lay_out = Conv3D(1,(1,1,1), activation='linear',name='output_layer')(lay_crop)
    
    return Model(lay_input,lay_out)

#%% 2.5D Version
def BlockModel25D(input_shape,filt_num=16,numBlocks=3):
    # Input shape should in the form
    # [rows,cols,slices,channels]
    lay_input = Input(shape=(input_shape),name='input_layer')
