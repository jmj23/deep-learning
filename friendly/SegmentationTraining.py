...
from Utils.JMJmodels import BlockModel2D
...
model_filepath = os.path.join(project_dir, "SegModel.hdf5")
...
def LoadData(datapath):
    ...
    for i in trange(x_data.shape[0], desc="Normalizing Data"):
        x_data[i] -= x_data[i].min()
        x_data[i] /= x_data[i].max()
    return x_data, y_data


# %% Data splitting into training and validation
def SplitData(x_data, y_data, val_split=0.2):
    # get total number of samples
    numSamples = x_data.shape[0]
    # select the last val_split fraction of samples as validation data
    val_inds = np.arange(numSamples - int(numSamples * val_split), numSamples)
...
def generate_augmented_data(inputs, targets):
    ...
    # gamma corrections
    gammas = 0.5 + np.random.rand(inputs.shape[0])
...
def dice_loss(y_true, y_pred):
    ...
    return 1 - (2.0 * intersection + 1) / (K.sum(y_true_f) + K.sum(y_pred_f) + 1)
...
def GetModel(x_train, numFilts=16, numBlocks=3):
    ...
    Model = BlockModel2D(x_train.shape[1:], numFilts, numBlocks, 3)
    adopt = optimizers.adam(lr=1e-4)
    Model.compile(loss=dice_loss, optimizer=adopt, metrics=[dice_score])
    return Model
...
def mask_viewer0(imvol, maskvol, name="Mask Display"):
    ...
    msk[..., 0] = 1
    msk[..., 1] = 1
    msk[..., 3] = 0.3 * maskvol.astype(float)
...
if __name__ == "__main__":
    ...
    save_masked_image(x_val[..., 0], output[..., 0])
