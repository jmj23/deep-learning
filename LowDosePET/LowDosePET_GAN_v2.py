...
model_filepath = 'LowDosePET_GAN_v2.hdf5'
# Data path/name
...
GMmodel = Model(inputs=GenModel.input, outputs=[DisModel([GenModel.input,GenModel.output]),GenModel.output])

GMmodel.compile(optimizer=adopt2, loss=['binary_crossentropy','mae'],loss_weights=[1,.2])
#%%
# Example Display function
def display_example(ex_ind,indata,fulldose_im,GenModel):
    plt.figure(1,figsize=(9,3.3),frameon=False)
    plt.ion()
    cond_samp = indata[np.newaxis,...]
    simfulldose_im = GenModel.predict(cond_samp)[...,0]
...
for ii in t:
    ...
    real_batch = y_train[batch_inds,...]
    gen_loss[ii] = GMmodel.train_on_batch(x=cond_batch,y=[y_batch,real_batch])
    if ii % progstep == 0:
        ...
        gg += 1
    t.set_postfix(Dloss=dis_loss[ii,0], Gloss=gen_loss[ii,0])
...
# display loss
fig2 = plt.figure(2)
plt.plot(np.arange(numIter),dis_loss,np.arange(numIter),gen_loss[:,0])
...
multi_slice_viewer0(progress_ims)
