...
savepath = 'lowdosePETdata.hdf5'

train_subj_vec = [4,5,7,8,9,10,11,12,13,15,17]
val_subj_vec = [2, 16]
test_subj_vec = [14, 3, 6]
...
def load_training_input(subj):
    waterpath = 'NIFTIs/subj{:03d}_WATER.nii'.format(subj)
    fatpath = 'NIFTIs/subj{:03d}_FAT.nii'.format(subj)
    LDpath = 'BREAST_RECON/lowdose/volunteer{:03d}_lowdose.nii.gz'.format(subj)
...
def load_training_target(subj):
    FDpath = 'BREAST_RECON/fulldose/volunteer{:03d}_fulldose.nii.gz'.format(subj)
