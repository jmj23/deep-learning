...
model_filepath = 'MuMapModel_Rep.hdf5'
model_keepname = 'MuMapModel_best.hdf5'
# Data path/name
...
for rep in range(numRep):
    ...
    RegModel = BlockModel_reg(x_train,dual_output,filterMult[rep])
    ...
    history = RegModel.fit(x_train,
                           ...
                           batch_size=b_s[rep], epochs=numEp,shuffle=True,
    ...
    score = RegModel.evaluate(x_val,{'reg_output': y_reg_val,'class_output':y_class_val})
    print("")
    print("Metrics on validation data")
    print("Weighted L1 loss: {:.03e}, Classification Loss: {:.02e}".format(score[1],score[2]))
    print("Model trained with filter multiplier {} and batch size {}".format(filterMult[rep],b_s[rep]))
    scores[rep] = [filterMult[rep],b_s[rep],score[0]]
    if score[0]< curBest:
        print('New best model found')
        curBest = score[0]
        os.rename(model_filepath,model_keepname)
        
#%% Display results of training
from tabulate import tabulate
print(tabulate(scores, headers=['Filter Mult', 'Batch Size', 'Score'],tablefmt='fancy_grid'))
#%%
print('Generating samples from best model')
TestModel = load_model(model_keepname,None,False)
# regression result
pr_bs = np.minimum(16,x_test.shape[0])
time1 = time.time()
output = TestModel.predict(x_test,batch_size=pr_bs)
time2 = time.time()
...
val_output = TestModel.predict(x_val,batch_size=pr_bs)
