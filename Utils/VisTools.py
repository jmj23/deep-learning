...
def DisplayDifferenceMask(im, mask1, mask2, name="Difference Mask", savepath=None):
    ...
    msk[mask_intersect, 0] = 0.2
    msk[mask_intersect, 1] = 0.8
    msk[mask_intersect, 2] = 0.2

    msk[mask1_only, 0] = 1
    msk[mask1_only, 1] = 1

    msk[mask2_only, 1] = 0.7
    msk[mask2_only, 2] = 1

    msk[..., 3] = 0.3 * mask_union.astype(float)
...
# %%
def YOLOviewer(im, target, conf=0.5):
...
def mask_viewer0(imvol, maskvol, maskvol2=None, name="Mask Display"):
    if maskvol2 is None:
        ...
        # mask different mask
        # adjust masks to binary
        maskvol1 = maskvol > 0.5
        maskvol2 = maskvol2 > 0.5
...
# %%
def registration_viewer(fixed, moving, alpha=0.5, name="Registration Results"):
